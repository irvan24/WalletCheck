...
import { isAddress } from "viem"; // ou 'ethers' si tu utilises Ethers.js
...
export default function Home() {
  ...
  const handleSubmit = (e) => {
    ...
    const isENS = address.endsWith(".eth") && address.length > 4;

    if (!isEthAddress && !isENS) {
      setError("Adresse Ethereum ou nom ENS invalide.");
      return;
    }

    // Valide : continue
    console.log("Scanning:", address);
    router.push(`/LoadingPage?address=${address}`)
  };
      ...
      <main className="relative z-10 flex flex-col items-center justify-center min-h-screen p-6">
        ...
        >
          ...
          <p className="text-xl text-gray-300 mb-8 max-w-2xl leading-relaxed">
            Analysez la santé de votre portefeuille Ethereum en quelques
            secondes. Découvrez vos tokens, autorisations et optimisez votre
            sécurité.
          </p>
        </div>

        <div
          className={`w-full max-w-2xl mb-16 transition-all duration-1000 delay-300 ${
            isAnimated
              ? "opacity-100 translate-y-0"
              : "opacity-0 translate-y-10"
          }`}
        ...
        <div
          className={`grid grid-cols-1 md:grid-cols-3 gap-8 mb-16 w-full max-w-4xl transition-all duration-1000 delay-600 ${
            isAnimated
              ? "opacity-100 translate-y-0"
              : "opacity-0 translate-y-10"
          }`}
        >
          {features.map((feature, index) => (
            <div
              key={index}
              className={`relative p-6 rounded-2xl bg-white/5 backdrop-blur-md border border-white/10 transition-all duration-500 hover:bg-white/10 hover:border-white/20 hover:scale-105 ${
                currentFeature === index
                  ? "ring-2 ring-yellow-500/50 bg-white/10"
                  : ""
              }`}
            >
              <div
                className={`w-12 h-12 rounded-xl bg-gradient-to-r from-white/10 to-white/5 flex items-center justify-center mb-4 ${feature.color}`}
              >
        ...
        <div
          className={`grid grid-cols-3 gap-8 w-full max-w-2xl transition-all duration-1000 delay-900 ${
            isAnimated
              ? "opacity-100 translate-y-0"
              : "opacity-0 translate-y-10"
          }`}
        ...
        <div
          className={`mt-12 flex items-center gap-4 text-sm text-gray-400 transition-all duration-1000 delay-1200 ${
            isAnimated
              ? "opacity-100 translate-y-0"
              : "opacity-0 translate-y-10"
          }`}
